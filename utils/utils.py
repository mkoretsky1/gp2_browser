...
def get_sidebar(obj):
    with st.sidebar:
        st.markdown(
            f'<p class="subheader-text">GP2 Release Version</p>',
            unsafe_allow_html=True,
        )
        obj.selected_release = st.selectbox(
            "Choose Release", ["8","7","6","5","4","3","2","1"], label_visibility="collapsed", on_change=update_master_key,
        )

        st.markdown(
            f'<p class="subheader-text">GP2 Cohort</p>',
            unsafe_allow_html=True,
        )
        master_key = st.session_state["master_key"]
        obj.selected_cohort = st.selectbox(
            "Choose Cohort",
            [f'GP2 Release {st.session_state["release_choice"]} FULL']+[study for study in master_key['study'].unique()],
            label_visibility="collapsed",
        )

        if obj.selected_release:
            st.session_state["release_choice"] = obj.selected_release
        if obj.selected_cohort:
            st.session_state["cohort_choice"] = obj.selected_cohort
...
def load_master_key() -> pd.DataFrame:
    ...
    master_key_path = (
        f"release_keys/master_key_release{release_choice}_app.csv"
    )
    try:
...
def filter_master_key(master_key: pd.DataFrame) -> pd.DataFrame:
    ...
    # Check if the selected cohort is the "FULL" option
    if cohort_choice == f'GP2 Release {release_choice} FULL':
        return master_key  # Return the entire master key without filtering

    if cohort_choice != "all":
        if cohort_choice not in master_key["study"].unique():
            st.error(f"Cohort {cohort_choice} is not available in Release {st.session_state['release_choice']}. Please select a different cohort.")
            st.stop()
